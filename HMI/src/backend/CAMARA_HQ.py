...
# Proceso de captura:
# 1. Inicialización de la cámara HQ con configuración específica.
# 2. Captura de frames en un hilo separado para no bloquear la ejecución principal.
# 3. Almacenamiento del frame actual en un atributo protegido por un lock para acceso seguro desde otros hilos.

# Proceso de medición:
# 1. Captura de frames desde la cámara HQ.
# 2. Extracción de un área de interés (ROI) centrada en la posición del láser.
# 3. Cálculo del diámetro del objeto en la ROI mediante detección de píxeles rojos.
# 4. Conversión del diámetro de píxeles a milímetros usando un factor de conversión.

# Proceso de transmisión web:
# 1. Redimensionamiento del frame capturado a las dimensiones web especificadas.
# 2. Dibujo de la ROI y del diámetro medido en el frame web.
# 3. Codificación del frame web a formato JPEG con la calidad especificada.
# 4. Generación de un flujo de frames JPEG para transmisión web.
...
class CAMARA_HQ:
    ...
    """
    # --- Inicialización de la clase CAMARA_HQ --
    def __init__(
        self,
    ...
    def __simulacion_loop(self):
        """
        Descripción: Bucle de simulación que genera un flujo de imágenes simuladas y un diámetro fijo para pruebas.
        Parámetros:
            None
        Retorna:
            None
    ...
    def __configurar(self, config):
        """
        Descripción: Configura la cámara con los parámetros especificados en el diccionario de configuración.
        Parámetros:
            config (dict): Diccionario con la configuración de la cámara.
        Retorna:
            None
        """
    ...
    def __capture_loop(self):
        """
        Descripción: Bucle de captura de frames desde la cámara.
        Parámetros:
            None
        Retorna:
            None
    ...
    def __diameter_loop(self):
        """
        Descripción: Bucle de cálculo del diámetro a partir del área de interés (ROI) centrada en la posición del láser.
        Parámetros:
            None
        Retorna:  
            None
    ...
    def __web_encoder_loop(self):
        """
        Descripción: Bucle de codificación para la transmisión web, que redimensiona el frame capturado, dibuja la ROI y el diámetro medido, y genera un flujo de frames JPEG.
        Parámetros:
            None
        Retorna:
            None
    ...
    def generate_frames(self):
        """
        Descripción: Genera frames JPEG para transmisión web, utilizando el frame codificado más reciente.
        Parámetros:
            None
        Retorna:
            bytes: Flujo de bytes JPEG para transmisión web.
        """
        while True:
            with self.lock_jpeg:
    ...
    def __calculate_diameter_from_roi(self, roi, medir_rojo=True):
        """
        Descripción: Calcula el diámetro a partir del área de interés (ROI) centrada en la posición del láser, detectando píxeles rojos y determinando la altura del objeto.
        Parámetros:
            roi (numpy.ndarray): Área de interés (ROI) centrada en la posición del láser.
            medir_rojo (bool): Indica si se debe medir el diámetro basado en píxeles rojos (True) o en ausencia de rojo (False).
        Retorna:
            tuple: (diametro_mm, mejor_inicio, mejor_fin) si se detecta un diámetro válido, o None si no se detecta un diámetro válido.
        """
    ...
    def __send_diameter(self, diametro_mm):
        """
        Descripción: Envía el diámetro medido a la salida estándar si está habilitado verbose, mostrando el valor del diámetro en milímetros.
        Parámetros:
            diametro_mm (float): Diámetro medido en milímetros.
        Retorna:
            None
        """
        if not self.verbose:
    ...
    def stop(self):
        """
        Descripción: Detiene la captura de la cámara y los hilos asociados, liberando recursos y finalizando la ejecución de la clase CAMARA_HQ.
        Parámetros:
            None
        Retorna:
            None
