...
# Descripción: Interfaz web para el control de la extrusora y enrolladora de filamento.
...
def manual():
    ...
    return html.Div(
        ...
        children=[
            html.Div([html.H3("Temperatura calefactor"), dcc.Slider(id="slider-temperatura", min=0, max=300, value=100, step=1, marks={0: "0°C", 0: "50°C", 50: "100°C", 180: "180°C", 220: "220°C", 250: "250°C", 400: "400°C"}, included=False, tooltip={"placement": "bottom", "always_visible": True}), html.Div([html.Button("ON", id="temperatura-on", n_clicks=0, className="btn-on"), html.Button("OFF", id="temperatura-off", n_clicks=0, className="btn-off")], className="contenedor-botones")], className="contenedor_slider_botones"),
...
def actualizar_svg_fsm(n):
    ...
    """
    # Leer posibles TEL:/OK:STATE pendientes antes de pintar. Así el SVG no
    # depende de que antes haya pasado el callback de monitorización.
    uc.leer_pendiente()

    estado_extrusora = int(uc.telemetria.get("state", 0) or 0)
    estado_winder = int(uc.telemetria.get("winder_state", 0) or 0)

    return (
        svg_extrusora_activa(estado_extrusora),
        svg_winder_activa(estado_winder),
    )
